    ...
    retval = [
        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        html.Span(
                            f"{us_percent_killed:,}% ", className="us-nat-death-num"
                        ),
                    ...
                    [
                        html.Div(
                            [
                                ...
                                " : ",
                                html.Span(
                                    f"{nat_children_deaths}", className="nat-stat-num"
                                ),
                            ]
                        ),
                        html.P(className="spacer"),
                        html.Div(
                            [
                                ...
                                " : ",
                                html.Span(
                                    f"{nat_teen_adult_deaths}", className="nat-stat-num"
                                ),
                            ]
                        ),
                        html.Div(
                            [
                                ...
                                " : ",
                                html.Span(
                                    f"{nat_adult_deaths}", className="nat-stat-num"
                                ),
                            ]
                        ),
                        html.Div(
                            [
                                ...
                                " : ",
                                html.Span(
                                    f"{nat_adv_adult_deaths}", className="nat-stat-num"
                                ),
                            ]
                        ),
                        html.Div(
                            [
                                ...
                                html.Span(
                                    f"{nat_senior_deaths}",
                                    className="nat-stat-num us-nat-death-num",
                                ),
                            ]
                        ),
                    ],
        ...
        html.P(
            [
                ...
                html.A(
                    "source CDC",
                    href="https://www.cdc.gov/pcd/issues/2021/21_0123.htm",
                    target="_blank",
                ),
                ").",
            ]
...
def get_data_tables(
    state_table_dict_df, state_table_age_dict_df, state_table_race_dict_df
) -> list:
    retval = [
        html.H2("Data Tables for All States"),
        html.P(
            "You can sort through each data category for all States. Some Territories have been excluded."
        ),
        html.P(className="spacer"),
        html.H4("Cases, Deaths, and Mortality Rates by State"),
        dash_table.DataTable(
            id="datatable-interactivity",
            columns=[{"name": i, "id": i} for i in state_table_dict_df.columns],
        ...
        dash_table.DataTable(
            ...
            columns=[{"name": i, "id": i} for i in state_table_age_dict_df.columns],
        ...
        html.Div(
            [
                html.P(
                    "Note: One or more age groups have counts between 1-9 and have been suppressed in accordance with NCHS confidentiality standards -they will appear as zero ('0')."
                ),
            ],
            className="section-div",
        ),
        ...
        dash_table.DataTable(
            ...
            columns=[{"name": i, "id": i} for i in state_table_race_dict_df.columns],
...
def get_about_covid_disease() -> list:
    retval = [
        ...
        html.P(
            "To begin to understand the threat an infectious disease like COVID-19 presents to the greater US population (and to each State), we must first declare and understand some underlying language, which is used to define some terms."
        ),
        html.P(className="spacer"),
        html.H5("COVID 'Case'"),
        html.P(
            "A COVID 'Case' is a recorded event where a person submits biological samples of cells from their respiratory system or blood to test for 1 of 2 types of tests."
        ),
        html.H6("Antigen Tests"),
        html.P(
            "Antigen tests basically look for a protein, which is typically found in a novel coronavirus (e.g. COVID-19). These tests are generally less expensive, but can also less reliable in some cases (e.g. if the testing person is not at the peak infection stage)."
        ),
        html.H6("Antibody Tests"),
        html.P(
            "Antibody tests basically look for antibodies that are developed from your body to defend against diseases similar to COVID-19. This means that, if your body has recently built a defense against an attack from a novel coronavirus (MERS, SARS CoV, SARS CoV-2 [AKA COVID-19]), or a disease similar to a coronavirus, like the Flu."
        ),
        html.P(className="spacer"),
        html.H5("COVID 'Death'"),
        html.P(
            "A COVID 'Death' is a recorded event where a person dies due to the direct effects of COVID-19 or if they have positive traces of COVID-19 in their body near, or at the time, of death."
        ),
        html.P(className="spacer"),
        html.H5("Mortality Rate"),
        html.P(
            "The lethality of infectious diseases are defined by the rate at which a \
            disease kills a population, which is often referred to as the disease's \
            'mortality rate'. After enough data is collected, these mortality \
            rates are calculated in order to assess the threat level the disease \
            presents to, either the immediate population from where the disease \
            emerged, or as large as a global scale if the disease's transmission rate and vector."
        ),
        dcc.Markdown(
            """
            Reference links
        ...
        html.P(className="spacer"),
        html.P(
            "This application has pulled COVID-19 data from the CDC and New York Times (see the 'Notes' section for source links) – which contains total COVID cases and death numbers. From here we can then calculate the averate national mortality rate for COVID, based on a simple percentage formula."
        ),
        html.P(className="spacer"),
        html.P(className="spacer"),
    ]
    ...
    retval = [
        ...
        html.Div(
            [
                dcc.Markdown(
                    """
                Have you ever asked yourself "How many children (0-12) have died from COVID?", \
                "What is the average survival rate?", or "How many Black or Asian people died from COVID in my state?"

                These are all questions I asked myself, shortly after the pandemic started. \
                I found that it was very hard to find a source that would easily explain COVID statistics \
                in 'plain English', so I sought out to create my own dashboard.

                Please feel free to check out the [National](/stats/national) statistics and/or \
                [State](/stats/state) page to look up stats for an \
                individual state. Please also refer to our [COVID definitions](/covid-definitions) page.

                For feedback or questions, please look up "Zero One Labs" on LinkedIn. Thank you!
            """,
                    id="purpose-div",
                ),
            ],
...
def build_state_graphs(
    ...
    BAR_GRAPH_HOVER_TEMPLATE = "%{y:,} <br>Date: %{x|%a, %b %d %Y}<br>"
    ...
    date_twodays_ago_str = datetime.strftime(
    ...
    date_fifty_nine_days_ago_str = datetime.strftime(
        date_fifty_nine_days_ago, "%Y-%m-%d"
    )
    ...
    def convert_totals_to_by_day(dataframe: pd.DataFrame) -> pd.DataFrame:
        prev_death = 0
        prev_cases = 0
        # fri_case_val = 0
        # fri_death_val = 0
        ...
        # zero_cases_days = 0
        zero_death_days = 0
        state_case_death_by_day = []
        dataframe = dataframe.sort_values(by="date")
        for i, row in dataframe.iterrows():
            # Added a function to make values positive because some
            # values from the dataframe contain negative values.
            # Don't know if this is an error in the NYT data reporting fewer
            # values the day after certain days, which seems to be throwing
            # off the algorithm.
            def make_positive(num) -> int:
                if num < 0:
                    num = num * -1
                return num

            # weekday_number = datetime.strptime(str(row['date']), "%Y-%m-%d 00:00:00").weekday()

            curr_death = make_positive(row["deaths"])
            curr_cases = make_positive(row["cases"])
            ...
            if prev_death == curr_death:
                adj_death = 0
            else:
                adj_death = curr_death - prev_death
                adj_death = abs(adj_death)
            prev_death = curr_death

            if prev_cases == curr_cases:
                adj_cases = 0
            else:
                adj_cases = curr_cases - prev_cases
                adj_cases = abs(adj_cases)
            ...
            # DISTRIBUTION_THRESHOLD = 5000

            if zero_death_days > 1 and adj_death > 0:
                # Threshold of days and value to check if needs to be divided
                # e.g. if the cases are 20,000 and the number of zero days are 4
                # then the threshold would be 5,000.
                if int(adj_death / zero_death_days) >= 1:
                    # print(f"Threshold matched! {adj_death}, {zero_death_days}")
                    adj_death = int(adj_death / zero_death_days)
                    for i in range(zero_death_days):
                        # Adjust the numbered index position's death count
                        # to be equal to the result of the division of the
                        # total deaths/cases over the number of zero-days
                        state_case_death_by_day[-(i + 1)]["deaths"] = adj_death
                    zero_death_days = 0

            state_case_death_by_day.append(
                {
                    "date": row["date"],
                    "state": row["state"],
                    "fips": row["fips"],
        ...
        new_dataframe = pd.DataFrame(state_case_death_by_day)
        return new_dataframe

    state_df = df.loc[(df["state"] == state_name)].copy()
    state_historical_daily_df = convert_totals_to_by_day(state_df)
    ...
    df_historical_2020 = state_historical_daily_df.loc[
        (state_historical_daily_df["date"] >= "2020-01-01")
        & (state_historical_daily_df["date"] <= "2020-12-31")
    ].copy()

    df_historical_2021 = state_historical_daily_df.loc[
        (state_historical_daily_df["date"] >= "2021-01-01")
        & (state_historical_daily_df["date"] <= "2021-12-31")
    ].copy()

    df_historical_2022 = state_historical_daily_df.loc[
        (state_historical_daily_df["date"] >= "2022-01-01")
        & (state_historical_daily_df["date"] <= "2022-12-31")
    ].copy()

    # def convert_totals_to_by_week(dataframe) -> list:
    #     # Making an assumption that the datafram being passed in is a complete yearly set of data
    #     prev_week = 0

    #     # Master list to return
    #     state_case_death_by_week = []

    #     # { "week_num": 00, "deaths": 00, "cases": 00 }
    #     weekly_case_death = {}

    #     for i, row in dataframe.iterrows():
    #         item_date = str(row['date'])
    #         week_number = datetime.strptime(item_date, "%Y-%m-%d 00:00:00").strftime("%U")

    #         if week_number not in weekly_case_death:
    #             weekly_case_death[week_number] = week_number

    #         curr_death = row['deaths']
    #         curr_cases = row['cases']

    #         if prev_week == week_number:
    #             weekly_case_death = { "week_num": week_number, "deaths": int(curr_death), "cases": int(curr_cases) }
    #         else:
    #             curr_death = curr_death + weekly_case_death["deaths"]
    #             curr_cases = curr_cases + weekly_case_death["cases"]
    #             weekly_case_death = { "week_num": week_number, "deaths": int(curr_death), "cases": int(curr_cases) }

    #         state_case_death_by_week.append(
    #             {"date": row['date'], "deaths": adj_death, "cases": adj_cases}
    #         )
    #     return state_case_death_by_week

    # state_case_death_by_day_2020_df = convert_totals_to_by_day(df_historical_2020)
    # state_case_death_by_day_2021_df = convert_totals_to_by_day(df_historical_2021)

    state_line_graph_line_2020 = px.bar(
        df_historical_2020,
        x="date",
        y=["deaths", "cases"],
        # color="y",
        title="Cases & Deaths by day, 2020",
        template=GRAPH_TEMPLATE,
    )
    state_line_graph_line_2020.update_layout(
        font=sd.line_legend_font_config, yaxis={"title": ""}, xaxis={"title": ""}
    )
    state_line_graph_line_2020.update_traces(hovertemplate=BAR_GRAPH_HOVER_TEMPLATE)

    state_line_graph_line_2021 = px.bar(
        df_historical_2021,
        x="date",
        y=["deaths", "cases"],
        title="Cases & Deaths by day, 2021",
        template=GRAPH_TEMPLATE,
    )
    state_line_graph_line_2021.update_layout(
        font=sd.line_legend_font_config, yaxis={"title": ""}, xaxis={"title": ""}
    )
    state_line_graph_line_2021.update_traces(hovertemplate=BAR_GRAPH_HOVER_TEMPLATE)
    try:
        state_line_graph_line_2021.add_vrect(
            x0=pd.Timestamp("2021-11-22 00:00:00"),
            x1=pd.Timestamp("2021-12-31 00:00:00"),
    ...
    state_line_graph_line_2022 = px.bar(
        df_historical_2022,
        x="date",
        y=["deaths", "cases"],
        # color="y",
        title="Cases & Deaths by day, 2022",
        template=GRAPH_TEMPLATE,
    )
    state_line_graph_line_2022.update_layout(
        font=sd.line_legend_font_config, yaxis={"title": ""}, xaxis={"title": ""}
    )
    state_line_graph_line_2022.update_traces(hovertemplate=BAR_GRAPH_HOVER_TEMPLATE)
    ...
    #     ].values[0]
    # except Exception as e:
    #     print(f"sixty_day_old_state_deaths: {e}")
    # try:
    ...
    #     ].values[0]
    # except Exception as e:
    #     print(f"state_last_sixty_deaths: {e}")
    # try:
    ...
    #     ].values[0]
    # except Exception as e:
    #     print(f"sixty_day_old_state_cases: {e}")
    # try:
    ...
    #     ].values[0]
    # except Exception as e:
    #     print(f"state_last_sixty_cases: {e}")
    ...
    # sixty_day_mortality_rate = round(
    #     (state_last_sixty_deaths / state_last_sixty_cases) * 100, 3
    # )
    ...
    ## No longer works because data is no longer being collected reliably 
    ## thus searching for data within the last 60 days will contain an empty list
    ## which throws an error later on when trying to access data in an empty list.
    
    # for state_neighbor in neighbors_list_long:
    #     df_state_single = df_historical_last_sixty_group.loc[
    #         df_historical_last_sixty_group["state"] == state_neighbor
    #     ]
    #     for case_type in ["cases", "deaths"]:
    #         state_oldest_case_val = df_historical_oldest_day.loc[
    #             df_historical_oldest_day["state"] == state_neighbor, case_type
    #         ].values[0]
    ...
    #                 (df_historical_last_sixty_group["state"] == state_neighbor)
    #                 & (df_historical_last_sixty_group["date"] == the_date),
    #                 case_type,
    #             ].values[0]
    ...
    # state_line_graph_linear_death_last_sixty = px.line(
    #     df_historical_last_sixty_group,
    #     x="date",
    #     y="deaths",
    #     color="state",
    #     title="Deaths",
    #     template=GRAPH_TEMPLATE,
    # )
    # state_line_graph_linear_death_last_sixty.update_layout(
    #     font=sd.line_legend_font_config
    # )

    # state_line_graph_linear_cases_last_sixty = px.line(
    #     df_historical_last_sixty_group,
    #     x="date",
    #     y="cases",
    #     color="state",
    ...
    #     template=GRAPH_TEMPLATE,
    # )
    # state_line_graph_linear_cases_last_sixty.update_layout(
    #     font=sd.line_legend_font_config
    # )

    # state_line_graph_log_death_last_sixty = px.line(
    #     df_historical_last_sixty_group,
    #     x="date",
    #     y="deaths",
    #     color="state",
    ...
    # state_line_graph_log_cases_last_sixty = px.line(
    #     df_historical_last_sixty_group,
    #     x="date",
    #     y="cases",
    #     color="state",
    ...
    for age_demo in data["Age"]:
        ...
        age_demo_death_dict["COVID Deaths"].append(
            data["Age"][age_demo]["total_deaths"]
        )
    ...
    for age_demo in data["Age"]:
        ...
        age_demo_death_pct_dict["Percent of Deaths"].append(
            data["Age"][age_demo]["pct_covid_deaths"]
        )
    ...
    grouped_age_demo_death_pct_dict = {
        "Age Group": ["Child", "Teen/Adult", "Adv Adult", "Senior"],
        "Percent of Deaths": [],
    }
    # Define variables
    child_death_pct, teenadult_death_pct, adult_death_pct, senior_death_pct = (
        0.0,
        0.0,
        0.0,
        0.0,
    ...
    age_death_bar_df = pd.DataFrame(
        age_demo_death_dict, columns=["Age Range", "COVID Deaths"]
    )
    ...
    age_death_bar.update_layout(
        font=sd.bar_legend_font_config, margin=sd.bar_graph_margins
    )
    ...
    age_death_pie.update_traces(
        textposition="inside", textinfo="percent+label", showlegend=False
    )
    age_death_pie.update_layout(
        font=sd.pie_legend_font_config, title=None, margin=sd.pie_graph_margins
    )
    ...
    grouped_age_death_pie.update_traces(
        textposition="inside", textinfo="percent+label", showlegend=False
    )
    grouped_age_death_pie.update_layout(
        font=sd.pie_legend_font_config, title=None, margin=sd.pie_graph_margins
    )
    ...
    for race_demo in data["Race"]:
        ...
        race_demo_death_pct_dict["Percent of Deaths"].append(
            data["Race"][race_demo]["pct_covid_deaths"]
        )
    ...
    for race_demo in data["Race"]:
        ...
        nonwhite_race_demo_death_pct_dict["Percent of Deaths"].append(
            data["Race"][race_demo]["pct_covid_deaths"]
        )

    # Create dataframes for bar graphs
    race_death_bar_df = pd.DataFrame(
        race_demo_death_dict, columns=["Race Group", "COVID Deaths"]
    )
    # Create bar graphs
    ...
    race_death_pie.update_traces(
        textposition="inside", textinfo="percent+label", showlegend=False
    )
    race_death_pie.update_layout(
        font=sd.pie_legend_font_config, title=None, margin=sd.pie_graph_margins
    )

    state_cases = sd.states_totals_df.loc[sd.states_totals_df["state"] == state_name][
        "cases"
    ].values[0]
    state_death = sd.states_totals_df.loc[sd.states_totals_df["state"] == state_name][
        "deaths"
    ].values[0]
    ...
    try:
        for state_id, teh_state in state_rank_dict["State"].items():
            state_rank_list.append(
                {
                    "state": teh_state,
                    "mortality_rate": state_rank_dict["Mortality Rate"][state_id],
                }
            )
        ...
        state_rank = state_rank_list.index(
            {"state": state_name, "mortality_rate": state_mrate}
        )
            ...
            [
                ...
                html.Div(
                    ...
                    {state_name} with {state_cases:,} cases and {state_death:,} deaths."""
                        ...
                        dcc.Markdown("""`((cases / deaths) * 100) = m`"""),
                        dcc.Markdown(
                            f"""the mortality rate (`m`) for {state_name} calculates to {state_mrate:,}% (inversely, \
                    the survival rate would be {state_survival_rate}%)."""
                        ),
                        html.P(className="spacer"),
                        html.P(className="spacer"),
                    #     html.P(
                    #         f"For the last ~60 days, {state_name} has seen {state_last_sixty_cases:,} \
                ...
                html.Div(
                    [
                        ...
                        html.P(
                            f"COVID cases for {state_name} make up {state_pct_of_natl_cases}% \
                ...
                html.Div(
                    [
                        dcc.Graph(
                            id=state_id_str + "-2020-line-graph",
                            className="state-figure-bar",
                            figure=state_line_graph_line_2020,
                            # config={
                            #     "scrollZoom": False,
                            #     "displayModeBar": False,
                            #     "responsive": True,
                ...
                html.Div(
                    [
                        dcc.Graph(
                            id=state_id_str + "-2021-line-graph",
                            className="state-figure-bar",
                            figure=state_line_graph_line_2021,
                            # config={
                            #     "scrollZoom": False,
                            #     "displayModeBar": False,
                            #     "responsive": True,
                ...
                html.Div(
                    [
                        dcc.Graph(
                            id=state_id_str + "-2022-line-graph",
                            className="state-figure-bar",
                            figure=state_line_graph_line_2022,
                            # config={
                            #     "scrollZoom": False,
                            #     "displayModeBar": False,
                            #     "responsive": True,
            ...
            accordance with NCHS confidentiality standards."
                ...
                html.Div(
                    [
                        ...
                        html.P(
                            f"The following graphs compare {state_name} versus their neighbors ({', '.join(x for x in neighbors_list_long)})."
                        ),
                        html.P(
                            "Note: If the state is Hawaii or Alaska, its listed neighbors are states with comparable populations."
                        ),
                        html.P(className="spacer"),
                        html.P(className="spacer"),
                        html.P(className="spacer"),
...
def get_footer(last_updated_race, last_updated_age) -> list:
    retval = [
        ...
        html.P(className="spacer"),
        html.P(
            "This project can only be as successful as the people who get involved and share this dashboard app. Please help me spread the word!"
        ),
        html.Ul(
            [
                html.Li(
                    html.A(
                        "Share to Facebook",
                        href="https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fcovid-state-demographic-data.herokuapp.com%2F",
                        target="_blank",
                    )
                ),
                html.Li(
                    html.A(
                        "Share to Twitter",
                        href="https://twitter.com/intent/tweet?text=Check out this awesome COVID data dashboard I found via @ZeroOneLabs: https%3A//covid-state-demographic-data.herokuapp.com/",
                        target="_blank",
                    )
                ),
                html.Li(
                    html.A(
                        "Share to LinkedIn",
                        href="https://www.linkedin.com/shareArticle?mini=true&url=https%3A//covid-state-demographic-data.herokuapp.com/&title=Check%20out%20this%20awesome%20COVID%20data%20dashboard!&summary=&source=",
                        target="_blank",
                    )
                ),
            ]
        ...
        html.H4("Notes"),
        html.P(
            "Please take care to notice and pay respect to the number of COVID deaths per state. Each bar graph displays the total, in a relative width, which may give an impresison the respective deaths are far greater than they really are. For example, one state may have a maximum of COVID deaths of around 23,000, while another state's max COVID deaths may be around 9,000."
        ),
        html.P(
            "State-wide data is retrieved nightly from the New York Times GitHub repository."
        ),
        html.P(
            "The information on this website is by no means to be taken as medical advice. If you have any serious concerns about your health, I urge you to contact your doctor or your healthcare maintenance organization (HMO). I advise you to follow your local and federal ordinances, in regards to any safety measures for mitigating the effects of COVID-19."
        ),
        html.P(className="spacer"),
        html.P(className="spacer"),
        ...
        html.P(
            [
                html.Span(
                    "Mortality rates are calculated, using the CDC's definition for the methodology of calculating the mortality rate of an infectious disease using the following formula:"
                ),
                html.Br(),
                html.Span(
                    "(deaths / cases) * 100 ~ per 100,000 people, or over a period of time (in this case the total span of the COVID pandemic)",
                    className="monospaced",
                ),
            ]
        ),
        html.P(
            "Data calcuated from the CDC does not include deaths with additional diseases, such as Pneumonia or Flu."
        ),
        html.P(className="spacer"),
        html.Hr(),
        html.Br(),
        ...
        html.P(
            [
                html.Li(
                    [
                        html.A(
                            "CDC: Provisional COVID-19 Deaths - Distribution of Deaths by Race and Hispanic Origin",
                            href="https://data.cdc.gov/NCHS/Provisional-COVID-19-Deaths-Distribution-of-Deaths/pj7m-y5uh",
                            target="_blank",
                        )
                    ]
                ),
                ...
                html.Li(
                    [
                        html.A(
                            "New York Times' GitHub COVID-19 Repository",
        ...
        html.P(
            [
                html.Span("This COVID-19 data dashboard was created by "),
                html.A(
                    "ZeroOneLabs.com", href="https://zeroonelabs.com", target="_blank"
                ),
            ]
        ),
        html.Br(),
